...
describe("computeStageScroll — stage", () => {
  ...
  it("returns 'underground' at the end of the forest→underground blend", () => {
    expect(computeStageScroll(3.1).stage).toBe("underground");
  });
...
describe("computeStageScroll — volumeMultiplier", () => {
  ...
  it("returns 0.5 at the start of the sky→forest blend", () => {
    expect(computeStageScroll(1.5).volumeMultiplier).toBeCloseTo(0.5);
  });
  it("ramps linearly inside the sky→forest blend", () => {
    // midpoint of [1.5, 1.8] is 1.65 → linear midpoint between 0.5 and 1.0 is 0.75
    expect(computeStageScroll(1.65).volumeMultiplier).toBeCloseTo(0.75);
  });
  ...
  it("ramps linearly inside the forest→underground blend", () => {
    // midpoint of [2.8, 3.1] is 2.95 → linear midpoint between 1.0 and 0.5 is 0.75
    expect(computeStageScroll(2.95).volumeMultiplier).toBeCloseTo(0.75);
  });
  it("returns 0.5 at the end of the forest→underground blend", () => {
    expect(computeStageScroll(3.1).volumeMultiplier).toBeCloseTo(0.5);
  });
...
describe("computeStageScroll — vignetteColor", () => {
  ...
  it("interpolates RGB channels in the sky→forest blend midpoint", () => {
    ...
    // (18,17,23.5). FP arithmetic on the boundary subtraction may land the
    // blend factor just under or just over 0.5; with the [1.5, 1.8] window
    // at scroll=1.65 the b channel rounds to 24.
  ...
  it("interpolates RGB channels in the forest→underground blend midpoint", () => {
    // midpoint between forest (26,20,16) and underground (14,9,5) is
    // mathematically (20,14.5,10.5); FP error in (2.95-2.8)/(3.1-2.8) lands
    // the blend factor just under 0.5 so g and b round down to 14 and 10.
    expect(computeStageScroll(2.95).vignetteColor).toBe("rgba(20, 14, 10, 0.85)");
  });
});
